...
execute() {
	pkg_installed_dir="/home/$SUDO_USER/.shaur/pkg-installed"

    printf "${LCYAN}[+]create shaur command${NC}\n";
    printf "Copying shaur.pl to... "
	sleep 1
	cp shaur.pl /usr/local/bin/shaur
    if [ $? -ne 0 ]; then
	...
	sleep 2
	cp -r shaurFunctions /usr/local/bin/shaurFunctions
    if [ $? -ne 0 ]; then
